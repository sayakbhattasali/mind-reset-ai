    ...
    <div className="fixed inset-0 w-full h-full bg-[#0E0E11] text-zinc-100 font-sans flex flex-col overflow-hidden select-none selection:bg-amber-500/30">
      ...
      <header className="shrink-0 h-14 sm:h-16 bg-[#0E0E11]/90 backdrop-blur-xl border-b border-zinc-800/80 z-30 px-4 sm:px-8 flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-1.5 sm:gap-2 text-zinc-400 hover:text-white transition-colors text-sm sm:text-sm font-medium focus:outline-none"
        >
          <ArrowLeft size={16} />
          <span>Exit</span>
        </button>
        ...
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-zinc-900 border border-zinc-800 flex items-center justify-center text-amber-400">
            <HeartPulse size={16} />
          </div>
          <span className="font-semibold text-base sm:text-lg text-zinc-100 tracking-tight">
            Mind<span className="text-amber-400">Reset</span>
          </span>
        </div>

        <div className="flex items-center gap-1.5 font-mono text-[11px] sm:text-xs text-zinc-400 uppercase tracking-wider">
      ...
      <main className="flex-1 min-h-0 w-full max-w-6xl mx-auto p-3 sm:p-4 lg:p-6 flex flex-col overflow-hidden">
        <div className="w-full h-full min-h-0 flex flex-col lg:grid lg:grid-cols-12 gap-3 sm:gap-4 lg:gap-6 items-stretch overflow-hidden">
          ...
          <section className="flex-[2] min-h-0 lg:h-full lg:col-span-5 flex flex-col justify-between bg-gradient-to-b from-[#16161A] via-[#121216] to-[#0E0E11] border border-zinc-800 rounded-2xl sm:rounded-3xl p-3 sm:p-4 lg:p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-zinc-900/90 border border-zinc-800 backdrop-blur-md">
                ...
                <span className="text-xs font-semibold text-zinc-200">
                  {isSpeaking ? "Dr. Marcus • Speaking" : "Dr. Marcus • AI"}
                </span>
              </div>
              <span className="text-[10px] sm:text-[11px] font-mono text-amber-400/80 uppercase tracking-wider hidden sm:inline">
                Somatic Edge
              </span>
            </div>
            ...
            <div className="relative z-10 pt-1.5 border-t border-zinc-800/80 flex items-center justify-between text-xs text-zinc-400 shrink-0">
              <div className="flex items-center gap-1.5 min-w-0">
                {isSpeaking ? (
                  ...
                  <span className="flex items-center gap-1.5 text-rose-400 font-medium animate-pulse truncate">
              ...
              <span className="font-mono text-[10px] text-zinc-500 shrink-0 ml-1 hidden sm:inline">Live Feedback</span>
            </div>
          </section>
            ...
            <AnimatePresence mode="wait">
              ...
              {phase === "calibrate" && (
                ...
                >
                  ...
                  <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-[#1C1D22] border border-zinc-800/80 space-y-2 text-center">
                    <div>
                      <div className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-amber-400 font-mono tracking-tight leading-tight">
                        {preScore} <span className="text-sm sm:text-base text-zinc-500 font-normal">/ 10</span>
                      </div>
                      <div className="text-xs sm:text-sm text-zinc-400 font-medium">
                        Urge Intensity (1 = mild, 10 = peak)
                      </div>
                    </div>
              ...
              {phase === "active" && (
                ...
                >
                  ...
                  <div className="shrink-0 flex items-center justify-between gap-2 pb-2 border-b border-zinc-800/80">
                    <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
                      <span className="hidden sm:inline text-sm font-semibold text-zinc-200 shrink-0">Session</span>
                      <span className="text-xs font-mono text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-0.5 rounded-full truncate">
                        {selectedTrigger} &bull; {preScore}/10
                      </span>
                    </div>
                    <button
                      onClick={endSession}
                      className="text-xs sm:text-sm font-semibold text-amber-400 hover:text-amber-300 transition-colors flex items-center gap-1.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/25 px-3 py-1.5 rounded-lg shrink-0 whitespace-nowrap"
                    >
                      <span>Wrap Up</span>
                      <ArrowRight size={13} />
                    </button>
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto bg-[#1C1D22] border border-zinc-800/80 rounded-xl sm:rounded-2xl p-3 sm:p-4 space-y-2.5 pr-2 scrollbar-thin">
                    {messages.length === 0 ? (
                      ...
                      messages.map((m, idx) => (
                        ...
                        >
                          <span className="text-[10px] sm:text-xs font-mono text-zinc-500 mb-0.5 px-1">
                    ...
                    {isListening && transcript && (
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] font-mono text-amber-400 mb-0.5 animate-pulse px-1">Transcribing...</span>
                        <div className="max-w-[88%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed bg-amber-500/10 text-amber-300 border border-amber-500/20 italic">
                  ...
                  <div className="shrink-0 space-y-1 pt-0.5">
                    <div className="flex gap-1.5 sm:gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 min-w-0 bg-[#1C1D22] border border-zinc-800 rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2.5 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-amber-500/50"
                      />
                      <button
                    ...
                    <div className="flex justify-between items-center text-[10px] sm:text-xs text-zinc-500 px-1 font-mono">
                      <span>Mic: {hasMicPermission ? "Connected" : "Standby"}</span>
                      <span>Speak or type anytime</span>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {phase === "rate_post" && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="flex-1 min-h-0 h-full flex flex-col justify-between space-y-3 sm:space-y-4 overflow-hidden"
                >
                  <div className="space-y-1">
                    <span className="text-xs sm:text-xs font-mono uppercase text-amber-400 tracking-wider">Clinical Re-Assessment</span>
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-zinc-100 tracking-tight leading-tight">How strong is the urge now?</h2>
                    <p className="text-xs sm:text-sm text-zinc-400">
                      Slide to record your post-session intensity.
                    </p>
                  </div>

                  <div className="p-4 sm:p-5 rounded-xl sm:rounded-2xl bg-[#1C1D22] border border-zinc-800/80 space-y-2 text-center my-auto">
                    <div className="text-4xl sm:text-5xl font-extrabold text-amber-400 font-mono tracking-tight leading-tight">
                      {postScore} <span className="text-sm sm:text-base text-zinc-500 font-normal">/ 10</span>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 sm:py-3.5 bg-[#FF8811] hover:bg-amber-400 text-zinc-950 font-bold rounded-xl sm:rounded-2xl transition-all shadow-[0_4px_25px_rgba(255,136,17,0.25)] text-sm sm:text-base flex items-center justify-center gap-2 active:scale-[0.98] shrink-0"
                  >
                    <span>Generate Relief Summary</span>
                    <ArrowRight size={16} />
                  </button>
                </motion.div>
              )}
              ...
              {phase === "done" && (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-zinc-400 font-mono mt-0.5">{selectedTrigger}</p>
                  </div>

                  {/* Pre/Post Urge Metrics Delta */}
                  <div className="grid grid-cols-3 gap-2 p-3 sm:p-4 border border-zinc-800 bg-[#1C1D22] rounded-xl sm:rounded-2xl font-mono">
                    <div>
                      <div className="text-[10px] sm:text-xs text-zinc-500 uppercase">Pre-Urge</div>
                      <div className="text-xl sm:text-2xl font-bold text-rose-400 mt-0.5">{preScore}/10</div>
                    </div>
                    <div>
                      <div className="text-[10px] sm:text-xs text-zinc-500 uppercase">Post-Urge</div>
                      <div className="text-xl sm:text-2xl font-bold text-amber-400 mt-0.5">{postScore}/10</div>
                    </div>
                    <div>
                      <div className="text-[10px] sm:text-xs text-zinc-500 uppercase">Relief</div>
                  ...
                  {/* Streak Claiming Box */}
                  {!currentUser ? (
                    <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-2.5 text-left">
                      <div>
                        <div className="text-sm sm:text-base font-semibold text-zinc-100 flex items-center gap-1.5">
                          <Sparkles size={14} className="text-amber-400 shrink-0" />
                          <span>Save this session?</span>
                        </div>
                        <p className="text-xs sm:text-sm text-zinc-400 mt-1 leading-snug">
                          Sign in with Google to log recovery trends, or continue as guest.
                        </p>
                      </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-2.5 px-3 bg-amber-400 hover:bg-amber-300 text-zinc-950 text-sm font-bold rounded-xl transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                        >
                          <LogIn size={13} />
                        ...
                        <button
                          ...
                          className="py-2.5 px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white text-sm font-medium rounded-xl transition-all"
                        >
                          Close
                        </button>
                      </div>
