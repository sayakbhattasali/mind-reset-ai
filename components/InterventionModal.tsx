...
import {
  ...
  X, Volume2, Activity, Flame, Smartphone, Sparkles,
...
export default function InterventionModal({
  isOpen,
  onClose,
  initialTrigger = "Substance Craving",
    ...
    try {
      ...
      await speakStream(
        ...
        (finalText, shouldEnd) => {
          ...
          if (shouldEnd) {
            setTimeout(() => setPhase("rate_post"), 800);
          } else {
            startListening();
          }
        }
      ...
      const fallbackMsg = "Take a slow, deep breath. I am right here with you.";
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md overflow-hidden">
        ...
        <motion.div
          ...
          className="relative w-full max-w-5xl h-[88vh] max-h-[760px] bg-[#0E0E11] border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col"
        >
          {/* Header */}
          <div className="shrink-0 h-14 bg-[#0E0E11]/90 backdrop-blur-xl border-b border-zinc-800/80 px-5 sm:px-8 flex items-center justify-between z-20">
            <div className="flex items-center gap-2">
              ...
              <span className="hidden sm:inline text-xs text-zinc-500 font-mono pl-2 border-l border-zinc-800">
                Dr. Marcus Somatic Intervention
              </span>
            </div>
          ...
          <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-12 gap-4 p-4 sm:p-6 overflow-hidden">
            
            {/* LEFT COLUMN: Avatar Studio with Somatic Breathing Aura */}
            <section className="lg:col-span-5 flex flex-col justify-between bg-gradient-to-b from-[#16161A] via-[#121216] to-[#0E0E11] border border-zinc-800 rounded-2xl p-5 relative overflow-hidden">
              ...
              <div className="relative z-10 my-auto flex items-center justify-center h-[230px] sm:h-[260px]">
              ...
              <div className="relative z-10 pt-3 border-t border-zinc-800/80 flex items-center justify-between text-xs text-zinc-400">
                <div className="flex items-center gap-2">
                  {isSpeaking ? (
                    <span className="flex items-center gap-1.5 text-amber-400 font-medium text-xs">
                      ...
                      <span>Speaking grounding guidance...</span>
                    </span>
                  ) : isListening ? (
                ...
                <span className="font-mono text-[10px] text-zinc-500">Live Voice</span>
              </div>
            </section>

            {/* RIGHT COLUMN: Dedicated Clinical Console */}
            <section className="lg:col-span-7 flex flex-col justify-between bg-[#16161A] border border-zinc-800 rounded-2xl p-4 sm:p-5 backdrop-blur-xl overflow-hidden">
              <AnimatePresence mode="wait">
                
                {/* STEP 1: Pre-Urge Calibration */}
                {phase === "calibrate" && (
                  <motion.div
                    key="calibrate"
                  ...
                  >
                    ...
                    {/* Compact Urge Intensity Meter */}
                    <div className="p-3.5 rounded-2xl bg-[#1C1D22] border border-zinc-800/80 space-y-2.5">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-zinc-300">
                          Urge Intensity Level
                        </span>
                        <div className="font-mono text-xl sm:text-2xl font-extrabold text-amber-400">
                          {preScore} <span className="text-xs text-zinc-500 font-normal">/ 10</span>
                        </div>
                      </div>
                          ...
                          >
                            ...
                            <div
                              className={`max-w-[88%] rounded-2xl px-4 py-2.5 text-xs sm:text-sm leading-relaxed ${
                                m.role === "user"
                                  ? "bg-amber-500/15 border border-amber-500/25 text-amber-100 rounded-tr-none shadow-sm"
                                  : "bg-zinc-900/90 border border-zinc-800 text-zinc-200 rounded-tl-none font-light shadow-sm"
                              }`}
                            >
                              {m.content}
                            </div>
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`p-3 rounded-2xl border transition-all shrink-0 ${
                            isListening
                              ? "bg-amber-500/20 border-amber-500/40 text-amber-400 animate-pulse shadow-[0_0_20px_rgba(245,158,11,0.2)]"
                        ...
                        <button
                          ...
                          disabled={isLoading}
                          className="p-3 bg-amber-400 hover:bg-amber-300 text-zinc-950 rounded-2xl font-bold transition-all disabled:opacity-40 shrink-0 shadow-[0_0_20px_rgba(245,158,11,0.15)]"
                        >
                          <Send size={16} />
                        </button>
                      </div>
                ...
                {phase === "rate_post" && (
                  ...
                  >
                    <div>
                      ...
                      <h2 className="text-2xl font-extrabold text-zinc-100 mt-1 tracking-tight">
                    ...
                    <button
                      ...
                      className="w-full py-3.5 bg-amber-400 hover:bg-amber-300 text-zinc-950 font-bold rounded-2xl transition-all shadow-[0_0_25px_rgba(245,158,11,0.15)] text-sm active:scale-[0.99]"
                    >
                ...
                {phase === "done" && (
                  ...
                  >
                    <div>
                      ...
                      <h2 className="text-xl font-bold text-zinc-100">Somatic Relief Verified</h2>
